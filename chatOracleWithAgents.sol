...
contract ChatOracle is IOracle {
    ...
    mapping(uint => Agent) private agents;
    uint private agentCount;

    function createAgent(string[] memory attributes, euint8[] memory ratings) public onlyOwner {
        require(attributes.length == ratings.length, "Attributes and ratings must have the same length");
        
        Agent storage newAgent = agents[agentCount];
        for (uint i = 0; i < attributes.length; i++) {
            newAgent.attributes.push(pair(attributes[i], ratings[i]));
        }
        newAgent.supervisor = msg.sender;
        agentCount++;
    }
    function addAttributeToAgent(uint agentId, string memory attribute, inEuint8 memory rating) public {
        require(msg.sender == agents[agentId].supervisor, "Only the supervisor can add attributes");
        agents[agentId].attributes.push(pair(attribute, FHE.asEuint8(rating)));

    }
    function getPrivateAgentStats(uint agentId) external view onlyOwner returns (bytes memory) {
        Agent storage agent = agents[agentId];
        uint8[] memory decryptedRatings = new uint8[](agent.attributes.length);
        string[] memory attributeNames = new string[](agent.attributes.length);

        for (uint i = 0; i < agent.attributes.length; i++) {
            decryptedRatings[i] = FHE.decrypt(agent.attributes[i].rating);
            attributeNames[i] = agent.attributes[i].attribute;
        }

        return abi.encode(attributeNames, decryptedRatings);
    }
    function getPublicAgentStats(uint agentId) external view returns (bytes memory) {
        Agent storage agent = agents[agentId];
        string[] memory attributeNames = new string[](agent.attributes.length);
        ...
        return abi.encode(attributeNames);
    }

    function getAgentCount() public view returns (uint) {
        return agentCount;
    }
}
